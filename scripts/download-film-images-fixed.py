#!/usr/bin/env python3
"""
Download film images from 48hourfilm.com by looking for download buttons with specific text patterns.

This script looks for download buttons with the following text patterns:
- "Behind the Scenes Pictures X" (where X is a number)
- "Film Stills X" (where X is a number)  
- "Poster X" (where X is a number)

The script parses the HTML to find these download buttons and downloads the associated images.
"""
...
def load_cookie(cookie_str: str) -> requests.cookies.RequestsCookieJar:
    """Parse cookie string into a RequestsCookieJar."""
...
def classify_image(link_text: str) -> Optional[Tuple[str, int]]:
    """
    Classify image based on download button text.
    Returns (image_type, number) or None if not a valid image type.
    
    Valid patterns:
    - "Behind the Scenes Pictures X" -> ("bts", X)
    - "Film Stills X" -> ("still", X)  
    - "Poster X" -> ("poster", X)
    """
    link_text = link_text.strip()
    
    # Behind the Scenes Pictures X
    match = re.search(r'Behind the Scenes Pictures (\d+)', link_text, re.IGNORECASE)
    if match:
        return ("bts", int(match.group(1)))
    
    # Film Stills X
    match = re.search(r'Film Stills (\d+)', link_text, re.IGNORECASE)
    if match:
        return ("still", int(match.group(1)))
    
    # Poster X
    match = re.search(r'Poster (\d+)', link_text, re.IGNORECASE)
    if match:
        return ("poster", int(match.group(1)))
    
    return None


def download_images(session: requests.Session, film_url: str) -> List[Tuple[str, str, int]]:
    """
    Find and download images from a film page.
    Returns list of (image_type, download_url, number) tuples.
    """
    ...
    # Look for download buttons in table rows
    for row in soup.find_all("tr"):
        cells = row.find_all("td")
        if len(cells) < 2:
            continue
        
        # First cell contains the descriptive text
        desc_cell = cells[0]
        desc_text = desc_cell.get_text(strip=True)
        
        # Second cell should contain the download link
        link_cell = cells[1]
        download_link = link_cell.find("a", text=re.compile(r'Download', re.IGNORECASE))
        
        if download_link and download_link.get("href"):
            # Classify the image based on the descriptive text
            classification = classify_image(desc_text)
            if classification:
                image_type, number = classification
                download_url = download_link["href"]
                
                # Make sure it's a full URL
                if download_url.startswith("/"):
                    download_url = "https://www.48hourfilm.com" + download_url
                elif not download_url.startswith("http"):
                    download_url = "https://www.48hourfilm.com/" + download_url
                
                images.append((image_type, download_url, number))
                print(f"  Found {image_type} {number}: {desc_text}")
...
def download_and_process_image(args_tuple: Tuple) -> Tuple[str, bool, str]:
    ...
    """
    film_name, image_type, download_url, number, dest_root, args = args_tuple
    ...
    try:
        ...
        # Create filename with correct numbering
        fname = f"{image_type}-{number}{ext}"
        ...
        return f"{film_name}:{fname}", False, f"Failed: {exc}"


def main() -> None:
    parser = argparse.ArgumentParser(description="Download 48 HFP film images from download buttons")
    ...
    with open(args.csv, newline="", encoding="utf-8") as f:
        ...
        for row in reader:
            ...
            try:
                # Find images based on download button text patterns
            ...
            if not images:
                print(f"No images found for {film_title}")
                continue
            ...
            download_tasks = []
            for image_type, download_url, number in images:
                download_tasks.append((film_title, image_type, download_url, number, dest_root, args))
