...
def classify_image_from_url(url: str) -> Optional[Tuple[str, int]]:
    """
    Classify image based on URL format and extract file number.
    Returns (image_type, number) or None if not a valid image type.
    
    The script looks for URLs in these specific formats:
    - Still: .../Film%20Stills/...file%20X.jpg
    - BTS: .../Behind%20the%20Scenes%20Pictures/...file%20X.jpg
    - Poster: .../Poster/...file%20X.jpg
    - Group: .../Group%20Picture/...file%20X.jpg
    """
    # First, properly decode the URL to handle %20 and other encodings
    decoded_url = unquote(url)
    
    # Look for the specific URL patterns
    if "/Film Stills/" in decoded_url:
        # Extract number from "file X" part
        match = re.search(r'file (\d+)', decoded_url)
        if match:
            return ("still", int(match.group(1)))
    
    elif "/Behind the Scenes Pictures/" in decoded_url:
        # Extract number from "file X" part
        match = re.search(r'file (\d+)', decoded_url)
        if match:
            return ("bts", int(match.group(1)))
    
    elif "/Poster/" in decoded_url:
        # Extract number from "file X" part
        match = re.search(r'file (\d+)', decoded_url)
        if match:
            return ("poster", int(match.group(1)))
    
    elif "/Group Picture/" in decoded_url:
        # Extract number from "file X" part
        match = re.search(r'file (\d+)', decoded_url)
        if match:
...
def find_upload_urls(session: requests.Session, film_url: str) -> List[Tuple[str, str, int]]:
    """
    ...
    for link in soup.find_all("a", href=True):
        ...
        if href.startswith("https://www.48hourfilm.com/uploads/2025/48HFP/48%20Hour%20Film%20Project%20-%20San%20Diego/"):
            ...
            classification = classify_image_from_url(href)
            if classification:
                image_type, number = classification
                upload_urls.append((image_type, href, number))
                print(f"  Found {image_type} {number}: {href}")
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Download 48 HFP film images from specific URL formats")
    ...
    with open(args.csv, newline="", encoding="utf-8") as f:
        ...
        for row in reader:
            ...
            download_tasks = []
            for image_type, download_url, number in upload_urls:
